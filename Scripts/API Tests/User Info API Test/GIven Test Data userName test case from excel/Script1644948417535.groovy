...
for (int i = 0; i <= lastRow; i++) {
	...
	//println(displayName + "," + expectedDisplayName)
	result = WS.verifyEqual(displayName, expectedDisplayName, FailureHandling.CONTINUE_ON_FAILURE)
	
	if (result == true) {
		CustomKeywords.'excel.WriteExcel.demoKey'(file, expectedDisplayName, "Expected Result", i)
		CustomKeywords.'excel.WriteExcel.demoKey'(file, displayName, "Actual Result", i)
